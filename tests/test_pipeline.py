...
def test_tokenizer(with_sample_doc):
    ...
    lines = [i for i in reader]
    assert len(lines) == 1
    assert lines[0] == ['Defendant', 'John', 'Doe', 'was', 'captured', 'by', 'U.S', 'Immigration', 'Services', '.']

def test_annotator(with_sample_doc):
    reader = Annotator(Tokenizer(AlephDumpReader(with_sample_doc)))
    lines = [i for i in reader]
    assert ('Defendant', 'NNP') in lines[0]
    assert Tree('PERSON', [('John', 'NNP'), ('Doe', 'NNP')]) in lines[0]
    assert Tree('ORGANIZATION', [('U.S', 'NNP'), ('Immigration', 'NNP'), ('Services', 'NNPS')]) in lines[0]
